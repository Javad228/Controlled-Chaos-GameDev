...
public class Room {
    ...
    private void initializeEnemies() {
        switch(roomNum) {
            ...
            case 3:
                ...
                enemies.add(new Barrel(650, 200));
                break;
            case 4:
    ...
    private void initializeCoins() {
        ...
        switch(roomNum) {
            case 1:
                ...
                Coin coin = new Coin(Coin.DEFAULT_FRAMES_TO_WAIT, Coin.DEFAULT_COIN_IMAGES, 625, 470, 1);
                coins.add(coin);
                break;
            case 8:
    ...
    public static ArrayList<Room> generateNewLevel(int difficulty, KeyHandler k, GamePanel g) {
        ...
        numOfRooms = DEFAULT_NUM_ROOMS + PlayerCharacter.EASY;
        ...
        if (difficulty == PlayerCharacter.RED_DOOR) initializeRandomItems(rooms.get(0), difficulty);
        ...
        // Always add a RapidFire item into room 0
        rooms.get(0).getItems().add(
                new RapidFire(
                        RapidFire.DEFAULT_IMAGE_PATHS,
                        g,
                        CoordinateWizard.getX(0),
                        CoordinateWizard.getY(0)
                )
        );
    ...
    private static void initializeRandomItems(Room r, int difficulty) {
        ...
        switch (difficulty) {
            case PlayerCharacter.GREEN_DOOR -> {
                numOfCommonItems = 0;
                numOfRareItems = 0;
                numOfEpicItems = 0;
            }
            case PlayerCharacter.RED_DOOR -> {
                numOfCommonItems = 1;
                numOfRareItems = 0;
                numOfEpicItems = 0;
        ...
        for (int i = 0; i < numOfCommonItems; i++) {
            ...
            if (commonItems[j].equals(Consumable.class)) {
                item = new Consumable(Sword.DEFAULT_IMAGE_PATHS, false, x, y);
            }
            else {
        ...
        for (int i = 0; i < numOfRareItems; i++) {
            ...
            if (rareItems[j].equals(HealthUp.class)) {
                item = new HealthUp(HealthUp.DEFAULT_HEALTHUP_PATHS, r.gp, x, y);
        ...
        for (int i = 0; i < numOfEpicItems; i++) {
            ...
            if (epicItems[j].equals(HealthUp.class)) {
                item = new HealthUp(HealthUp.DEFAULT_HEALTHUP_PATHS, r.gp, x, y);
            } else if (epicItems[j].equals(BombBuddy.class)) {
                item = new BombBuddy(BombBuddy.DEFAULT_BOMB_BUDDY_PATHS, r.gp, x, y);
            } else {
    ...
    private static void initializeRandomEnemies(Room r, int difficulty) {
        ...
        switch (difficulty) {
            case PlayerCharacter.GREEN_DOOR -> minNumOfEnemies = 2;
            case PlayerCharacter.RED_DOOR -> minNumOfEnemies = 4;
            default -> {
                System.err.printf("initializeRandomEnemies() - Player Difficulty not valid: %d\n", difficulty);
                minNumOfEnemies = 0;
                variability = 0;
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (enemyTypes[j].equals(Skeleton.class)) {
                e = new Skeleton(x, y);
            } else {
                e = new Wizard(x, y);
            }
            ...
            e.setMaxHealth((difficulty == PlayerCharacter.GREEN_DOOR) ? e.getMaxHealth() : ((int)(e.getMaxHealth() * 1.5)));
            e.setDamagePerHit((difficulty == PlayerCharacter.GREEN_DOOR) ? e.getDamagePerHit() : ((int)(e.getDamagePerHit() * 1.5)));
    ...
    public static void initializeRandomCoins(Room r, int difficulty) {
        ...
        switch (difficulty) {
            case PlayerCharacter.GREEN_DOOR, PlayerCharacter.RED_DOOR -> value = 1;
            default -> {
                System.err.printf("initializeRandomCoins() - Player Difficulty not valid: %d\n", difficulty);
                minNumOfCoins = 0;
                variability = 0;
